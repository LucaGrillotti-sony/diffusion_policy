...
class EnvControlWrapperJTC:
    def __init__(self, jtc_client, n_obs_steps, n_action_steps, path_bag_robot_description):
        ...
        offset_xyz = np.asarray([0.40996018, 0.03893278, 0.45212647])
        offset_angle = np.pi / 4
        radius = 0.05
        gamma = np.pi / 4
        position_xyz = offset_xyz + np.asarray([radius * np.cos(gamma) * np.cos(offset_angle), radius * np.sin(gamma) * np.cos(offset_angle), radius * np.sin(offset_angle)])

        initial_eef = np.asarray(
            [*position_xyz, 0.0673149, 0.96574436, 0.2338243, 0.03675712]
        )
